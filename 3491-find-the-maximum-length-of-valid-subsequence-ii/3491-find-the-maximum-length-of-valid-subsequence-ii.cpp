...
public:
    int maximumLength(vector<int>& A, int k) {
        ...
        rep(i,0,sz(A)) mp[A[i]%k].push_back(i);
        vll ord;
        for(auto& [k,v] : mp) {
            res = max(res, sz(v));
            ord.push_back(k);
        ...
        rep(i,0,sz(ord)) {
            rep(j,i+1,sz(ord)) {
                ll a = ord[i], b = ord[j];
                if(sz(mp[a]) + sz(mp[b]) <= res) continue;
                for(ll i = 0, j = 0, now = 0, at = -1, fl = 0;;) {
                    if(fl == 0) {
                        while(i < sz(mp[a]) and mp[a][i] < at) i++;
                        if(i == sz(mp[a])) break;
                        now++;
                        fl = !fl;
                        at = mp[a][i];
                    } else {
                        while(j < sz(mp[b]) and mp[b][j] < at) j++;
                        if(j == sz(mp[b])) break;
                        now++;
                        fl = !fl;
                        at = mp[b][j];
