...
class Solution {
    ll helper(vs& A, ll n, ll m, ll cj, ll ca, ll mj, ll mo, ll fl) {
        if(fl == 150) return false;
        if(vis[fl][ca][mo] != -1) return vis[fl][ca][mo];
        if(ca == mo) return false;
        if(A[ca / m][ca % m] == 'F') return false;
        if(A[mo / m][mo % m] == 'F') return true;
        ll& res = vis[fl][ca][mo] = !(fl&1);
        if(fl & 1) {
            ll y = mo / m, x = mo % m;
            rep(i,0,4) {
                rep(d,0,mj+1) {
                    ll ny = y + dy[i] * d, nx = x + dx[i] * d;
                    if(0 <= ny and ny < n and 0 <= nx and nx < m and A[ny][nx] != '#') {
                        if(helper(A,n,m,cj,ca,mj,ny * m + nx, fl + 1)) return res = true;
                    } else break;
                }
            }
        } else {
            ...
            rep(i,0,4) {
                rep(d,0,cj+1) {
                    ll ny = y + dy[i] * d, nx = x + dx[i] * d;
                    if(0 <= ny and ny < n and 0 <= nx and nx < m and A[ny][nx] != '#') {
                        if(!helper(A,n,m,cj,ny * m + nx,mj,mo, fl + 1)) return res = false;
                    } else break;
                }
            }
        }
...
public:
    bool canMouseWin(vector<string>& grid, int catJump, int mouseJump) {
        MINUS(vis);
        ll cat = 0, mouse = 0, n = sz(grid), m = sz(grid[0]);
        rep(i,0,n) rep(j,0,m) {
                if(grid[i][j] == 'C') cat = i * m + j;
                if(grid[i][j] == 'M') mouse = i * m + j;
            }

        return helper(grid,n,m,catJump,cat,mouseJump,mouse,1);
    }
};
