...
struct Seg{
    ll mi, ma, cnt;
    Seg *left, *right;
    Seg(vll& A, ll l, ll r) : mi(A[l]), ma(A[r]), cnt(0), left(nullptr), right(nullptr) {
        if(l ^ r) {
            ll m = l + (r - l) / 2;
            left = new Seg(A,l,m);
    ...
    ll query(ll n) {
        if(mi > n) return cnt;
        if(ma <= n) return 0;
        return left->query(n) + right->query(n);
    }
};
...
public:
    vector<int> resultArray(vector<int>& A) {
        vll S(all(A));
        uniq(S);
        Seg* s1 = new Seg(S,0,sz(S) - 1);
        Seg* s2 = new Seg(S,0,sz(S) - 1);
        vi a1{A[0]}, a2{A[1]};
        s1->update(A[0]);
        s2->update(A[1]);
        rep(i,2,sz(A)) {
        ...
        rep(i,0,sz(a2)) a1.push_back(a2[i]);
        return a1;
    }
};