...
ll LCA[MAX_N][22], dp[MAX_N][22][27], level[MAX_N];
ll query(ll u, ll v) {
    if(level[u] < level[v]) swap(u, v);
    ll diff = level[u] - level[v];
    for(ll i = 0; diff; i++, diff /= 2) {
        if(diff & 1) u = LCA[u][i];
    }
...
int ask(ll u, ll k, ll w) {
    ll res = 0;
    for(ll i = 0; k and i < 22; i++, k /= 2) {
        if(k & 1) {
            res += dp[u][i][w];
            u = LCA[u][i];
        }
...
class Solution {
    vector<vpll> adj;
    void dfs1(ll u, ll par, ll dep = 0) {
        level[u] = dep;
        for(auto [v,w] : adj[u]) {
            if(v == par) continue;
            LCA[v][0] = u;
    ...
    vector<int> minOperationsQueries(int n, vector<vector<int>>& edges, vector<vector<int>>& queries) {
        ...
        rep(j,0,22) {
            rep(i,0,n) {
                ...
                rep(k,1,27) {
                    dp[i][j+1][k] = dp[LCA[i][j]][j][k] + dp[i][j][k];
                }
            }
        }
        vi res;
        rep(i,0,sz(queries)) {
            ...
            int lca = query(u,v), dis = distance(u,v), now = dis;
            int disu = distance(lca,u), disv = distance(lca,v);
            rep(j,1,27) {
                now = min(now, dis - ask(u,disu,j) - ask(v,disv,j));
            }
            res.push_back(now);
        }
        return res;
