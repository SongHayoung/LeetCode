...
public:
    int countKSubsequencesWithMaxBeauty(string s, int k) {
        ...
        vll ord;
        for(auto [k,v] : freq) ord.push_back(v);
        sort(begin(ord), end(ord));
        if(sz(ord) < k) return 0;
        ll res = 1;
        while(k) {
            int cnt = 0, best = ord.back();
            while(ord.size() and ord.back() == best) {
                ord.pop_back(); cnt += 1;
            }
            if(cnt <= k) {
                res = res * modpow(best, cnt) % mod;
                k -= cnt;
            } else {
                res = res * modpow(best, k) % mod * comb.nCr(cnt,k) % mod;
                k = 0;
            }
        }
        return res;
