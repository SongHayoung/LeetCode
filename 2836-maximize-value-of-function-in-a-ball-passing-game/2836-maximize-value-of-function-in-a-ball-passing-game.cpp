...
vll st;
ll rep[MAX_N], sum[MAX_N], cnt[MAX_N], vis[MAX_N], id = 1;
ll dp[MAX_N][24], dis[MAX_N], who[MAX_N][24], rootOf[MAX_N];
bool isCycle(ll u) {
    if(rep[u] != u) return true;
    if(cnt[u] != 1) return true;
    if(adj[u][0] == u) return true;
    return false;
}
void dfs1(ll u) {
    vis[u] = id;
...
void setup(ll n) {
    for(int j = 0; j < 23; j++) {
        for(int i = 0; i < n; i++) {
            if(who[i][j] == -1) continue;
            who[i][j+1] = who[who[i][j]][j];
            dp[i][j+1] = dp[who[i][j]][j] + dp[i][j];
        }
    }
}
void bfs(ll n, ll k) {
    queue<ll> q;
    auto push = [&](ll u, ll c, ll par) {
        if(!isCycle(u)) {
            q.push(u);
            dis[u] = c;
            rootOf[u] = rootOf[par];
...
ll disQuery(ll u, ll k) {
    ll res = 0;
    for(ll i = 0; k and i < 24; i++, k /= 2) {
        if(k & 1) {
            res += dp[u][i];
            u = who[u][i];
        }
...
ll ncycleQuery(ll u, ll k) {
    return disQuery(u,k);
}
ll cycleQuery(ll u, ll k) {
    ll ccnt = cnt[rep[u]], res = 0;
    res += sum[rep[u]] * (k / ccnt);
    k %= ccnt;
    if(k) {
        res += disQuery(u,k);
    }
...
ll query(ll u, ll k) {
    if(isCycle(u))
        return cycleQuery(u,k);
    if(dis[u] >= k)
        return ncycleQuery(u,k);
    ll where = rootOf[u];
    return ncycleQuery(u,dis[u]) + cycleQuery(where, k - dis[u]);
}
