...
ll seg1[MAX_N * 4], seg2[MAX_N * 4], lz[MAX_N * 4], at[MAX_N];
void update(ll u, ll l, ll r, ll k) {
    seg2[u] = (seg2[u] + 2 * seg1[u] * k + k * k * (r - l + 1)) % mod;
    seg1[u] = (seg1[u] + (r - l + 1) * k) % mod;
    lz[u] += k;
}
void add(ll u, ll s, ll e, ll l, ll r) {
    if(e < l or s > r) return;
    if(l <= s and e <= r) {
        seg2[u] = (seg2[u] + 2 * seg1[u] + e - s + 1) % mod;
        seg1[u] = (seg1[u] + e - s + 1) % mod;
        lz[u] += 1;
         return;
    }
    ll m = s + (e - s) / 2;
    if(lz[u] > 0) {
        update(2 * u, s, m, lz[u]);
        update(2 * u  + 1, m + 1, e, lz[u]);
        lz[u] = 0;
    }
    add(2 * u, s, m, l, r);
    add(2 * u + 1, m + 1, e, l, r);
    seg2[u] = (seg2[2 * u] + seg2[2 * u + 1]) % mod;
    seg1[u] = (seg1[2 * u] + seg1[2 * u + 1]) % mod;
}
class Solution {
public:
    int sumCounts(vector<int>& A) {
        for(auto& x : A) at[x] = 0;
        rep(i,0,sz(A) * 4) 
            seg1[i] = seg2[i] = lz[i] = 0;
        ll res = 0;
        rep(i,1,sz(A)+1) {
            add(1,1,sz(A),at[A[i-1]] + 1, i);
            res = (res + seg2[1]) % mod;
            at[A[i-1]] = i;
        }
        return res;
