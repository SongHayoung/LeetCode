...
struct Seg {
    ll mi, ma, who;
    Seg *left, *right;
    Seg(vi& A, ll l, ll r) : mi(A[l]), ma(A[r]), who(INF), left(nullptr), right(nullptr) {
        if(l ^ r) {
            ll m = l + (r - l) / 2;
            left = new Seg(A,l,m);
...
public:
    vector<int> leftmostBuildingQueries(vector<int>& heights, vector<vector<int>>& queries) {
        ...
        vall3 Q;
        rep(i,0,sz(queries)) {
            ll u = queries[i][0], v = queries[i][1];
            if(u < v) swap(u,v);
            Q.push_back({u,v,i});
        }
        sort(all(Q));
        ...
        ll p = sz(heights) - 1;
        while(sz(Q)) {
            auto [u,v,idx] = Q.back(); Q.pop_back();
            while(p > u) {
                seg->update(heights[p], p);
                p -= 1;
            }
            if(heights[v] < heights[u]) res[idx] = u;
            else if(u == v) res[idx] = u;
            else {
                ll p = seg->query(heights[v]);
                if(p != INF) res[idx] = p;
            }
        }
        return res;
