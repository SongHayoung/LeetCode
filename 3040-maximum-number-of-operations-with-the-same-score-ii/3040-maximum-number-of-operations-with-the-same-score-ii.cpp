...
ll dp[2020][2020];
class Solution {
    ll helper(vi& A, ll target, ll l, ll r) {
        if(l >= r) return 0;
        if(dp[l][r] != -1) return dp[l][r];
        ll& res = dp[l][r] = 0;
        if(A[l] + A[l+1] == target)
            res = max(res, 1 + helper(A,target, l + 2, r));
        if(A[r] + A[r-1] == target)
            res = max(res, 1 + helper(A, target, l, r - 2));
        if(A[l] + A[r] == target)
            res = max(res, 1 + helper(A,target, l + 1, r - 1));
        return res;
    }
public:
    int maxOperations(vector<int>& A) {
        usll us;
        us.insert(A[0] + A[1]);
        us.insert(A.back() + A[0]);
        us.insert(A.back() + A[sz(A)-2]);
        ll res = 0;
        for(auto x : us) {
            MINUS(dp);
            res = max(res, helper(A,x,0,sz(A) - 1));
        }
        return res;
    }
};