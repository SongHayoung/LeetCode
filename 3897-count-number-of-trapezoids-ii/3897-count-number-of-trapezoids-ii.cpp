...
struct Line {
    ...
    pair<string , string> formulaS() {
        T y1 = A.y, y2 = B.y;
        T x1 = A.x, x2 = B.x;
        if(y1 == y2) return {"+", to_string(y1)};
        if(x1 == x2) return {"-", to_string(x1)};
        ll a = (y2 - y1), b = (x2 - x1), g = __gcd(a,b);
        a /= g, b /= g;
...
public:
    int countTrapezoids(vector<vector<int>>& points) {
        int n = sz(points);
        vpll P;
        rep(i,0,n) P.push_back({points[i][0], points[i][1]});
        vall3 freq, rfreq;
        rep(i,0,n) rep(j,i+1,n) {
                ll x1 = P[i].first, y1 = P[i].second;
                ll x2 = P[j].first, y2 = P[j].second;
                ll dy = y2 - y1, dx = x2 - x1;
                ll g = __gcd(llabs(dy), llabs(dx));
                dy /= g; dx /= g;
                if (dx < 0 || (dx == 0 && dy < 0)) { dy = -dy; dx = -dx; }
                ll c = dx * y1 - dy * x1;
                freq.push_back({dy,dx,c});
                ll mx = x1 + x2, my = y1 + y2;
                ll midKey = (mx << 32) ^ (my & 0xffffffff);
                rfreq.push_back({midKey,dy,dx});
        }
        ll res = 0;
        sort(all(freq));
        sort(all(rfreq));
        while(sz(freq)) {
            ll a = freq.back()[0], b = freq.back()[1], prv = INF, sum = 0, now = 0;
            while(sz(freq) and freq.back()[0] == a and freq.back()[1] == b) {
                auto [_,_,c] = freq.back(); freq.pop_back();
                if(c != prv) {
                    sum += now;
                    now = 0;
                    prv = c;
        ...
        while(sz(rfreq)) {
            ll a = rfreq.back()[0], tot = 0, sum = 0, now = 0;
            pll prv{-INF, -INF};
            while(sz(rfreq) and rfreq.back()[0] == a) {
                auto [_, x,y] = rfreq.back(); rfreq.pop_back();
                pll c{x,y};
                if(prv != c) {
                    sum += now * (now - 1) / 2;
                    now = 0;
