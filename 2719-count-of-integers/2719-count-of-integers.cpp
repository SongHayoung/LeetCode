...
ll dp[33][444];
class Solution {
    bool ok(string s, int mi, int ma) {
        int sum = 0;
        for(auto& ch : s) sum += ch - '0';
        return mi <= sum and sum <= ma;
    }
    ll helper(string& s, ll mi, ll ma, ll sum, ll p, ll x, bool lo) {
        if(p == sz(s)) return mi <= sum and sum <= ma;
        if(sum > ma) return 0;
        if(!lo) {
            ll res = 0;
            rep(i,0,s[p]-'0') {
                res = (res + helper(s,mi,ma,sum + i, p + 1, x - 1, true)) % mod;
            }
            res = (res + helper(s,mi,ma,sum + s[p] - '0', p + 1, x - 1, false)) % mod;
            return res;
        } else {
            ll& res = dp[p][sum];
            if(res != -1) return res;
    ...
    ll helper(string n, ll mi, ll ma) {
        MINUS(dp);
        return helper(n,mi,ma,0,0,sz(n) - 1,false);
    }
public:
    int count(string num1, string num2, int min_sum, int max_sum) {
        ll res = helper(num2, min_sum, max_sum) - helper(num1, min_sum, max_sum);
        return (res + ok(num1,min_sum, max_sum) + mod) % mod;
    }
};
