...
class Solution {
    uspll us;
    bool ok(vector<int>& A, vector<int>& B, ll c) {
        int y = A[0], x = A[1], cnt = 0, wall = 0;
        uspll vis;
        queue<pll> q;
        auto push = [&](int y, int x) {
            if(!vis.count({y,x}) and !us.count({y,x})) {
                vis.insert({y,x});
                q.push({y,x});
            }
        };
        ...
        while(q.size() and cnt <= c) {
            ll qsz = sz(q);
            while(qsz--) {
                auto [y,x] = q.front(); q.pop();
                rep(i,0,4) {
                    ll ny = y + dy[i], nx = x + dx[i];
                    if(0 <= ny and ny <= 999999 and 0 <= nx and nx <= 999999) {
                        push(ny,nx);
                    }
                }
            }
        ...
        return sz(q) or vis.count({B[0],B[1]});
    }
public:
    bool isEscapePossible(vector<vector<int>>& B, vector<int>& S, vector<int>& T) {
        us = {};
        for(auto& b : B) us.insert({b[0],b[1]});
        return ok(S,T,sz(B) * 2) and ok(T,S,sz(B) * 2);
    }
};
