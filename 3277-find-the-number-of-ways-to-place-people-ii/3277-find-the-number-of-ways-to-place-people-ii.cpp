...
public:
    int numberOfPairs(vector<vector<int>>& A) {
        ...
        ll res = 0;
        for(auto i = rbegin(mp); i != rend(mp); i++) {
            for(auto x = begin(i->second); x != end(i->second); x++) {
                ll hold = INF;
                if(next(x) != end(i->second)) hold = *next(x), res += 1;
                for(auto j = next(i); j != rend(mp) and hold > *x; j++) {
                    auto lb = j->second.lower_bound(*x);
                    if(lb == end(j->second)) continue;
                    if(*lb >= hold) continue;
                    res += 1;
                    hold = *lb;
                }
