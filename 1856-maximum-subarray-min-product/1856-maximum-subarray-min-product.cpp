...
struct Seg {
    ...
    pll val;
    Seg *left, *right;
    Seg(vi& A, ll l, ll r) : mi(l), ma(r), val({INF,INF}), left(nullptr), right(nullptr) {
        if(l^r) {
            ll m = l + (r - l) / 2;
            left = new Seg(A,l,m);
    ...
    pll query(ll l, ll r) {
        ...
        if(mi > r or ma < l) return {INF,INF};
        return min(left->query(l,r), right->query(l,r));
    }
};
class Solution {
    ll dnc(vll& pre, Seg* seg, ll l, ll r) {
        if(l > r) return 0;
        auto [mi,idx] = seg->query(l,r);
        ll res = mi * (pre[r] - (l ? pre[l-1] : 0));
        return max({res, dnc(pre,seg,l,idx-1), dnc(pre,seg,idx+1,r)});
    }
public:
    int maxSumMinProduct(vector<int>& nums) {
        Seg* seg = new Seg(nums,0,sz(nums)-1);
        vll pre{nums[0]};
        rep(i,1,nums.size()) pre.push_back(nums[i] + pre.back());
        return dnc(pre,seg,0,sz(nums) - 1) % mod;
    }
};