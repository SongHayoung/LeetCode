...
class Solution {
    ll len(pll A, pll B) {
        return max(0ll, min(B.second,A.second) - max(A.first,B.first) + 1);
    }
public:
    int numberOfWays(int n, vector<int>& limit) {
        vll S{1,n-1,n,n+1,n+2};
        rep(i,0,sz(limit)) {
            ll x = limit[i];
            S.push_back(x);
            S.push_back(x-1);
        ...
        uniq(S);
        umll at;
        rep(i,0,sz(S)) at[S[i]] = i;
        vll pre(sz(S));
        rep(i,0,sz(limit)) {
            pre[at[1]] += 1;
            pre[at[min(limit[i],n) + 1]] -= 1;
        }
        rep(i,1,sz(pre)) pre[i] += pre[i-1];
        vll suf(sz(S));
        rrep(i,0,sz(S) - 2) {
            suf[i] = (suf[i+1] + pre[i] * (S[i+1] - S[i]) % mod) % mod;
        }
        ll res = 0;
        rep(i,0,sz(limit)) {
            ll x = limit[i];
            ll r = n - 1, l = max(1ll, n - x);
            ll cnt = suf[at[l]] - suf[at[r + 1]] - len({l,r},{1,x}) + mod;
            res = (res + cnt) % mod;
        }
        return res;
    }
