...
vll pi(string& p) {
    vll PI(sz(p));
    for(ll i = 1, j = 0; i < sz(p); i++) {
        while(j and p[i] != p[j]) j = PI[j-1];
        if(p[i] == p[j]) PI[i] = ++j;
    }
    return PI;
}
vll kmp(string& s, string& p) {
    vll PI = pi(p);
    vll res;
...
public:
    vector<int> beautifulIndices(string s, string a, string b, int k) {
        vll aa = kmp(s,a), bb = kmp(s,b);
        dqll A(all(aa)), B(all(bb));
        vi res;
        while(sz(A) and sz(B)) {
            while(sz(B) and B.front() < A.front() - k) B.pop_front();
            if(sz(B) == 0) break;
            if(abs(A.front() - B.front()) <= k) res.push_back(A.front());
            A.pop_front();
        }
        return res;
    }
