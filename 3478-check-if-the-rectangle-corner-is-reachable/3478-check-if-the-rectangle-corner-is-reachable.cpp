...
class Solution {
    bool match(vector<int>& A, vector<int>& B) {
        Circle<ll> a(A[0], A[1], A[2]);
        Circle<ll> b(B[0],B[1],B[2]);
        return a.intersect(b);
    }
public:
    bool canReachCorner(int X, int Y, vector<vector<int>>& circles) {
        ASCEND(uf);
        vall4 mark(sz(circles));
        Point<ll> S(0,0), E(X,Y);
        rep(i,0,sz(circles)) {
            rep(j,i+1,sz(circles)) {
                if(match(circles[i], circles[j])) uni(i,j);
            }
            Circle<ll> c(circles[i][0], circles[i][1], circles[i][2]);
            if(c.include(S) or c.include(E)) return false;
        ...
        rep(i,0,sz(circles)) {
            ll p = find(i);
            rep(j,0,4) mark[p][j] |= mark[i][j];
        }
        rep(i,0,sz(circles)) {
            if(mark[i][0] and mark[i][2]) return false;
            if(mark[i][1] and mark[i][3]) return false;
            if(mark[i][0] and mark[i][1]) return false;
            if(mark[i][2] and mark[i][3]) return false;
        }
        return true;
    }
};