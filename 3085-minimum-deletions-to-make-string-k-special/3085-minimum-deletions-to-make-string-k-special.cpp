...
class Solution {
    ll helper(vll& freq, ll x, ll k) {
        ll res = 0, ma = *max_element(all(freq));
        rep(i,0,sz(freq)) {
            if(!freq[i]) continue;
            if(freq[i] < ma - k){
                res += freq[i];
...
public:
    int minimumDeletions(string word, int k) {
        vll freq(26);
        rep(i,0,sz(word)) freq[word[i]-'a']++;
        ll res = INF, base = 0;
        vll S = freq;
        freq = S;
        sort(all(S));
        while(sz(S) and S.back() > 0) {
            res = min(res, base + helper(freq,S.back(), k));
            ll cur = S.back();
            while(sz(S) and S.back() == cur) S.pop_back();
            if(sz(S) == 0 or S.back() == 0) break;
            rep(i,0,sz(freq)) {
                if(freq[i] > S.back() + k) {
                    base += freq[i] - (S.back() + k);
                    freq[i] = S.back() + k;
                }
            }
        }
