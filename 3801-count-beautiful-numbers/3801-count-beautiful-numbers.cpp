...
umll dp[8][66];
class Solution {
    ll helper(ll x, ll prod, ll sum) {
        if(x == 0) return prod % sum == 0;
        if(dp[x][sum].count(prod)) return dp[x][sum][prod];
        ll& res = dp[x][sum][prod] = 0;
        rep(i,0,10) {
            res += helper(x-1, prod * i, sum + i);
        }
        return res;
    }
    ll helper(string& s, ll pos, bool less, bool leading, ll prod, ll sum) {
        if(pos == sz(s)) return !leading and prod % sum == 0;
        if(less) {
            ll res = 0;
            if(leading) {
                res += helper(s,pos+1,true,true,0,0);
                rep(i,1,10) {
                    res += helper(sz(s) - pos - 1, i, i);
                }
            } else {
                res += helper(sz(s) - pos, prod, sum);
            }
            return res;
        }
        ll res = helper(s,pos+1,false,false,pos ? (s[pos]-'0') * prod : s[pos] - '0', sum + s[pos] - '0');
        rep(i,0,s[pos] - '0') {
            res += helper(s,pos+1,true,!pos and i == 0, pos ? i * prod : i, sum + i);
        }
        return res;
    }
    ll helper(ll n) {
        if(!n) return 0;
        rep(i,0,8) rep(j,0,66) dp[i][j] = {};
        string s = to_string(n);
        return helper(s,0,false,true,0,0);
    }
public:
