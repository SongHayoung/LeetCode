...
struct Trie {
    ...
    void insert(string& s, ll idx, ll p = 0) {
        tot++;
        if(sz(s) == p) {
            who.push_back(idx);
        } else {
...
class Solution {
    ...
    ll dfs(vector<vector<char>>& A, ll y, ll x, Trie* t, vs& res, vs& W) {
        if(!t) return 0;
        char ch = A[y][x]; A[y][x] = '#';
        ll ret = sz(t->who);
        for(auto& w : t->who) res.push_back(W[w]);
        t->who = {};
        ll n = sz(A), m = sz(A[0]);
        rep(i,0,4) {
            ll ny = y + dy[i], nx = x + dx[i];
            if(0 <= ny and ny < n and 0 <= nx and nx < m and isalpha(A[ny][nx])) {
                ret += dfs(A,ny,nx,t->next[A[ny][nx]-'a'],res,W);
            }
        }
...
public:
    vector<string> findWords(vector<vector<char>>& board, vector<string>& words) {
        ...
        vs res;
        rep(i,0,sz(board)) rep(j,0,sz(board[0])) {
            dfs(board,i,j,t->next[board[i][j] - 'a'],res,words);
            if(t->next[board[i][j]-'a'] and t->next[board[i][j]-'a']->tot == 0) t->next[board[i][j]-'a'] = nullptr;
        }
        return res;
    }
};
