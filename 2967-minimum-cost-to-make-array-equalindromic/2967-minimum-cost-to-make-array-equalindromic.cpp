...
class Solution {
    ...
    ll tot = 0, tcnt = 0;
    ll query(ll x) {
        auto it = prev(pre.upper_bound(x));
        auto [lc, ls] = it->second;
        auto rc = tcnt - lc;
        auto rs = tot - ls;
        return lc * x - ls + rs - rc * x;
    }
public:
    long long minimumCost(vector<int>& nums) {
        ...
        auto add =[&](ll x) {
            ...
            ll len = sz(s), cut = len / 2;
            rep(i,0,cut) s.pop_back();
            ll val = stoll(s);
            while(1) {
                ll now = make(val, cut);
                if(now <= x) {
                    c.push_back(now);
                    c.push_back(make(val + 1,cut));
                    break;
                }
                val -= 1;
            }
        ...
        for(auto& [k,p] : pre) {
            tcnt += p.first, tot += p.second;
            p = {tcnt,tot};
            if(k) {
                add(k);
            }
        }
        rep(i,1,10) c.push_back(i);
        ll res = INF;
        for(auto& x : c) res = min(res, query(x));
        return res;
    }
};
