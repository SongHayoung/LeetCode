...
class Solution {
    ...
    ll dfs(ll u, ll par, ll fl, vi& A) {
        if(dp[fl][u] != -INF) return dp[fl][u];
        ll& res = dp[fl][u] = 0;
        if(fl) {
            for(auto& v : adj[u]) {
                if(v == par) continue;
        ...
        if(sz(adj[u]) == 1 and adj[u][0] == par) return res = -INF;
        res = A[u];
        for(auto& v : adj[u]) {
            if(v == par) continue;
            res += max(dfs(v,u,fl,A),dfs(v,u,!fl,A));
            res = max(res, -INF);
        }
        return res;
    }
public:
    long long maximumScoreAfterOperations(vector<vector<int>>& edges, vector<int>& values) {
        ...
        dfs1(0,-1,values);
        return max(dfs(0,-1,true,values), dfs(0,-1,false,values));
    }
};