...
public:
    int xorAfterQueries(vector<int>& nums, vector<vector<int>>& queries) {
        ...
        vector<unordered_map<long long, long long>> qry(n);

        auto add = [&](int idx, int jump, long long val){
            if(idx >= n) return ;
            if(!qry[idx].count(jump)) qry[idx][jump] = 1;
            qry[idx][jump] = qry[idx][jump] * val % mod;
            if(qry[idx][jump] == 1) qry[idx].erase(jump);
        };

        auto udt = [&](int l, int r, int jump, int val) {
            add(l,jump,val);
            add(((r - l) / jump + 1) * jump + l, jump, modpow(val, mod-2));
        };
        for(auto& q : queries) {
            udt(q[0],q[1],q[2],q[3]);
        }
