...
class Solution {
    all3 helper(vvi& A, ll exc) {
        pll sum{INF, -INF}, sumi{0,0};
        pll diff{INF,-INF}, diffi{0,0};
        rep(i,0,sz(A)) {
            if(i == exc) continue;
            if(sum.first > A[i][0] + A[i][1]) {
                sum.first = A[i][0] + A[i][1];
                sumi.first = i;
            }
            if(sum.second < A[i][0] + A[i][1]) {
                sum.second = A[i][0] + A[i][1];
                sumi.second = i;
            }
            if(diff.first > A[i][0] - A[i][1]) {
                diff.first = A[i][0] - A[i][1];
                diffi.first = i;
            }
            if(diff.second < A[i][0] - A[i][1]) {
                diff.second = A[i][0] - A[i][1];
                diffi.second = i;
            }
        }
        if(sum.second - sum.first > diff.second - diff.first) {
            return {sum.second - sum.first, sumi.first, sumi.second};
        }
        return {diff.second - diff.first, diffi.first, diffi.second};
    }
public:
    int minimumDistance(vector<vector<int>>& A) {
        auto [_,x,y] = helper(A,-1);
        return min(helper(A,x)[0], helper(A,y)[0]);
    }
};