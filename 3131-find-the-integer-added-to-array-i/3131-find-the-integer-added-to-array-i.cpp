...
public:
    int addedInteger(vector<int>& A, vector<int>& B) {
        ...
        rep(i,0,sz(B)) fb[B[i]]++;
        vpll sa,sb;
        for(auto& [k,v] : fa) sa.push_back({k,v});
        for(auto& [k,v] : fb) sb.push_back({k,v});
        sort(all(sa));
        sort(all(sb));
        bool ok = true;
        rep(i,0,sz(sa)) {
            if(sa[i].second != sb[i].second) ok = false;
        }
        if(ok) return sb[0].first - sa[0].first;
        return sb[0].first - sa.back().first;
    }
};