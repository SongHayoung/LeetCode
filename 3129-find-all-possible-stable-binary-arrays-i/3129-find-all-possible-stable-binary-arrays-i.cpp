    ...
    int numberOfStableArrays(int zero, int one, int limit) {
        ...
        rep(i,1,zero + one) {
            ...
            rep(fl,0,2) {
                rep(lim,1,limit) {
                    rep(o,0,one+1) {
                        if(!dp[fl][lim][o]) continue;
                        ll oc = o, zc = (i - o);
                        if(oc + 1 <= one) {
                            if(fl == 0) {
                                dpp[1][1][oc+1] = (dpp[1][1][oc+1] + dp[fl][lim][o]) % mod;
                            } else if(lim + 1 < limit) {
                                dpp[1][lim+1][oc+1] = (dpp[1][lim+1][oc+1] + dp[fl][lim][o]) % mod;
                            }
                        }
                        if(zc + 1 <= zero) {
                            if(fl == 1) {
                                dpp[0][1][oc] = (dpp[0][1][oc] + dp[fl][lim][o]) % mod;
                            } else if(lim + 1 < limit) {
                                dpp[0][lim + 1][oc] = (dpp[0][lim + 1][oc] + dp[fl][lim][o]) % mod;
                            }
                        }
                    }
                }
