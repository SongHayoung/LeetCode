...
class Solution {
    ll helper(all4& x, all4& y) {
        ll mi = INF;
        mi = min(mi, max(0ll, min(y[2],x[2]) - max(x[0],y[0])));
        mi = min(mi, max(0ll, min(y[3],x[3]) - max(x[1],y[1])));
        return mi * mi;
    }
public:
    long long largestSquareArea(vector<vector<int>>& b, vector<vector<int>>& t) {
        ...
        ll res = 0, n = sz(b);
        rep(i,0,n) {
            A.push_back({b[i][0], b[i][1], t[i][0], t[i][1]});
        }
        sort(all(A));
        rep(i,0,n) rep(j,i+1,n) {
            if(A[i][2] <= A[j][0]) break;
            res = max(res, helper(A[i], A[j]));
        }
        return res;
    }
};