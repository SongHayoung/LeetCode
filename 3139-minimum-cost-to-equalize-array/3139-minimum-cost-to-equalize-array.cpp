...
public:
    int minCostToEqualizeArray(vector<int>& A, int C1, int C2) {
        ...
        if(sz(A) == 1) return 0;
        sort(all(A));
        if(sz(A) == 2) return 1ll * c1 * (A[1] - A[0]) % mod;
        if(c1 * 2 <= c2) {
            ll res = 0;
            rep(i,0,sz(A)) {
                res = (res + 1ll * (A.back() - A[i]) % mod * c1 % mod) % mod;
            }
            return res;
        }
        auto single = [&]() {
        ...
        vll S;
        rep(i,0,sz(A)) S.push_back(A.back() - A[i]);
        sort(all(S));
        ll sum = 0;
        rep(i,0,sz(S) - 1) sum += S[i];
        ll tot = sum + S.back(), n = sz(A);
        if(sum >= S.back()) {
            if(tot % 2 == 0) {
                return tot / 2 % mod * c2 % mod;
            }
            ll res = tot / 2 % mod * c2 % mod;
            res = (res + single()) % mod;
            return res;
        }
        ll res = sum % mod * c2 % mod;
        ll rem = S.back() - sum;
        ll best = c1 * rem, op = 0;

        while(rem > (n - 1) and op < best) {
            ll d = rem / (n - 1);
            rem = rem - d * (n - 1);
            rem = rem + d;
            op = (op + d * (n - 1) * c2);
            best = min(best, op + rem * c1);
        }
        if(rem == 0 or op >= best) return (res + min(best, op) % mod) % mod;
        ll oth = n - rem;
        ll mi = rem * c1;

        {
            ll omi = oth / 2 * c2 % mod + rem * c2;
            if(oth & 1) {
                ll x = single();
                omi += x;
            }
        ...
        return (res + min(best, op + mi) % mod) % mod;
    }
};