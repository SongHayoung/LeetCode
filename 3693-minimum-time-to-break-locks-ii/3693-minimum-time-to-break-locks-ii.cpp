...
ll source = MAX_N - 2, sink = MAX_N - 1;
ll cost[MAX_N][MAX_N], flo[MAX_N][MAX_N], cap[MAX_N][MAX_N], path[MAX_N];
vll adj[MAX_N];
...
public:
    int findMinimumTime(vector<int>& A) {
        ZERO(cap), ZERO(flo), ZERO(cost);
        rep(i,0,MAX_N) adj[i].clear();
        ll n = sz(A);
        rep(i,0,n) {
            rrep(j,1,n) {
                ll c = A[i] / j + (A[i] % j == 0 ? 0 : 1);
                connect(i + 1, n + j, 1, c);
                if(!i) connect(n + j, sink, 1, 0);
            }
            connect(source, i + 1, 1, 0);
        }
        ll res = 0, flow = 0;
