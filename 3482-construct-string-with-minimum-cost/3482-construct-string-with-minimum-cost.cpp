...
struct Trie {
    ...
    void insert(string& s, ll c, ll p = 0) {
        if(sz(s) == p) {
            cst = min(cst, c);
        } else {
            if(!nxt[s[p]-'a']) nxt[s[p]-'a'] = new Trie();
            nxt[s[p]-'a']->insert(s,c,p+1);
        }
    }
};
...
class Solution {
    ...
    ll helper(string& s, ll p) {
        ...
        for(int i = p; i < s.length(); i++) {
            ...
            if(!now->nxt[ch]) break;
            now = now->nxt[ch];
            if(now->cst != INF) res = min(res, helper(s,i+1) + now->cst);
        }
