...
struct Seg {
    ...
    void pushUp() {
        best = {0,0,0,0};
        rep(i,0,4) rep(j,0,4) {
            if(BIT(i,0) and BIT(j,1)) continue;
            ll k = j % 2 + (BIT(i,1)<<1);
            best[k] = max(best[k], left->best[i] + right->best[j]);
        }
    }
    Seg(vi& A, ll l, ll r) : mi(l), ma(r), left(nullptr), right(nullptr), best({0,0,0,0}) {
        if(l^r) {
            ll m = l + (r - l) / 2;
            left = new Seg(A,l,m);
    ...
    void update(ll n, ll k) {
        if(mi <= n and n <= ma) {
            if(n == ma and n == mi) best = {0,0,0,max(0ll,k)};
            else {
                left->update(n,k);
                right->update(n,k);
                pushUp();
    ...
    ll pick() {
        return max({best[0], best[1], best[2], best[3]});
    }
};
class Solution {
public:
