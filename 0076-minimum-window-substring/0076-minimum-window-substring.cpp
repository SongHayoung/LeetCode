...
public:
    string minWindow(string s, string t) {
        unordered_map<char,ll> freq, us;
        for(auto& ch : t) us[ch]++;
        ll ok = 0;
        auto push = [&](char ch) {
            ++freq[ch];
                if(us.count(ch) and freq[ch] == us[ch]) ok++;
            
        };
        auto pop = [&](char ch) {
            --freq[ch];
                if(us.count(ch) and freq[ch] + 1 == us[ch]) ok--;
        };
        ll l = 0, r = 0, n = sz(s), res = INF, at = INF;
        while(r < n) {
            while(r < n and ok != sz(us)) push(s[r++]);
            while(l < n and ok == sz(us)) {
                ll now = r - l;
                if(now < res) res = now, at = l;
                pop(s[l++]);
            }
        }

        return res == INF ? "" : s.substr(at,res);
    }
};
