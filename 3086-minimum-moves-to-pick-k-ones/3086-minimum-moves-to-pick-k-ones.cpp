...
struct Seg {
    ll mi,ma,cnt, cost;
    Seg *left, *right;
    Seg(vi& A, ll l, ll r) : mi(l), ma(r), cnt(0), cost(0), left(nullptr), right(nullptr) {
        if(l ^ r) {
            ll m = l + (r - l) / 2;
            left = new Seg(A,l,m);
        ...
        } else if(A[l]) cnt = A[l], cost = l;
    }
    pll query(ll l, ll r) {
        if(l <= mi and ma <= r) return {cnt, cost};
        if(mi > r or l > ma) return {0ll, 0ll};
        auto [lc, lcst] = left->query(l,r);
        auto [rc, rcst] = right->query(l,r);
        return {lc + rc, lcst + rcst};
    }
...
public:
    long long minimumMoves(vector<int>& A, int k, int maxChanges) {
        ...
        auto qry = [&](ll pos) {
            ...
            if(target) {
                ll use = min(target, maxChanges * 1ll);
                res += 2 * use;
                target -= use;
            }
            if(target) {
