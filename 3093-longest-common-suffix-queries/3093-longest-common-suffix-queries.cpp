...
struct Trie {
    ...
    void insert(string& s, pll& cost, ll p) {
        best = min(best, cost);
        if(p == -1) return;
        if(!next[s[p]-'a']) next[s[p]-'a'] = new Trie();
        next[s[p]-'a']->insert(s,cost,p-1);
    }
    ll query(string& s, ll p) {
        if(p == -1) return best.second;
        if(!next[s[p]-'a']) return best.second;
...
public:
    vector<int> stringIndices(vector<string>& wordsContainer, vector<string>& wordsQuery) {
        ...
        rep(i,0,sz(wordsContainer)) {
            pll now{sz(wordsContainer[i]), i};
            t->insert(wordsContainer[i],now, sz(wordsContainer[i]) - 1);
        }
        vi res;
        rep(i,0,sz(wordsQuery)) {
            res.push_back(t->query(wordsQuery[i], sz(wordsQuery[i]) - 1));
        }
        return res;
    }
};