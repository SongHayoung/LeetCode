...
public:
    int sumOfPowers(vector<int>& A, int k) {
        sort(all(A));
        vector<vector<umll>> dp(k,vector<umll>(sz(A)));
        rep(i,0,sz(A)) {
            rrep(size,0,k-2) {
                rep(pos,0,i) {
                    ll diff = abs(A[i] - A[pos]);
                    for(auto& [key,value] : dp[size][pos]) {
                        dp[size+1][i][min(key,diff)] = (dp[size+1][i][min(key,diff)] + value) % mod;
                    }
                }
            }
            dp[0][i][INF] = 1;
        ...
        ll res = 0;
        rep(i,0,sz(A)) {
            for(auto& [key, value] : dp[k-1][i]) {
                res = (res + key % mod * value % mod) % mod;
            }
        }
        return res;
    }
