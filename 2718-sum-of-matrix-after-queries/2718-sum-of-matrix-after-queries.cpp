...
struct Seg{
    ll mi,ma,val,lazy;
    Seg *left, *right;
    Seg(ll l, ll r, ll x) : mi(l), ma(r), val(x), lazy(0), left(nullptr), right(nullptr) {
        if(l ^ r) {
            ll m = l + (r - l) / 2;
            left = new Seg(l,m,x);
            right = new Seg(m+1,r,x);
        }
    }
    void ludt() {
        if(lazy) {
            val = max(0ll, val - lazy);
    ...
    void update(ll l, ll r, ll x) {
        ...
        if(r < mi or l > ma) return;
        if(left) left->update(l,r,x);
        if(right) right->update(l,r,x);
    }
    ll query(ll n) {
        ludt();
        if(mi == n and n == ma) return val;
        if(mi <= n and n <= ma) return left->query(n) + right->query(n);
        return 0;
    }
};
class Solution {
public:
    long long matrixSumQueries(int n, vector<vector<int>>& queries) {
        ...
        for(int i = queries.size() - 1; i >= 0; i--) {
            int op = queries[i][0], idx = queries[i][1], x = queries[i][2];
            if(op == 0) {
                ll cnt = r->query(idx);
                if(!cnt) continue;
                res += cnt * x;
                r->update(idx, idx, cnt);
                c->update(0,n-1,1);
            }
            if(op == 1) {
                ll cnt = c->query(idx);
