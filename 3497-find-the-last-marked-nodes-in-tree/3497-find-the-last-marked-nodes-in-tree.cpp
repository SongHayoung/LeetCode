...
pll who[MAX_N];
class Solution {
    pll dfs1(vvll& adj, ll u, ll par) {
        who[u] = {0,u};
        for(auto& v : adj[u]) {
            if(v == par) continue;
            auto sub = dfs1(adj,v,u);
            sub.first++;
            who[u] = max(who[u], sub);
        }
        return who[u];
    }
    void dfs2(vvll& adj, ll u, ll par, pll pars, vi& res) {
        pll a = {0,u}, b = pars;
        if(b > a) swap(a,b);
        for(auto& v : adj[u]) {
            if(v == par) continue;
        ...
        for(auto& v : adj[u]) {
            if(v == par) continue;
            auto nxt = who[v].second == a.second ? b : a;
            nxt.first++;
            dfs2(adj,v,u,nxt,res);
        }
    }
public:
