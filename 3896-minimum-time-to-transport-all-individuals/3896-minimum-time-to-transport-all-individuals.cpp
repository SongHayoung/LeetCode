...
class Solution {
    vll bits;
    ll done;
    ll getCost(vi& A, ll bit) {
        ll res = 0;
        rep(i,0,sz(A)) if(BIT(bit,i)) res = max(res, 1ll * A[i]);
        return res;
    }
public:
    double minTime(int N, int K, int M, vector<int>& time, vector<double>& mul) {
        if(K == 1) return N == 1 ? time[0] * mul[0] : -1.;
        if(N == 1) return time[0] * mul[0];
        done = 1<<N;
        vector<vector<vld>> dp(done, vector<vld>(M, vld(2,INF)));
        bits = {};
        rep(i,1,done) {
            ll pc = __builtin_popcount(i);
            if(pc <= K) bits.push_back(i);
        }
        priority_queue<array<ld,4>,vector<array<ld,4>>, greater<>> q;
        auto push = [&](ll mask, ll cyc, ll fl, ld c) {
            if(dp[mask][cyc][fl] > c) {
                dp[mask][cyc][fl] = c;
                q.push({c,(ld)mask,(ld)cyc,(ld)fl});
            }
        };
        push(0,0,0,0);
        while(sz(q)) {
            auto [cost, ldmask, ldcyc, ldfl] = q.top(); q.pop();
            ll mask = ldmask, cyc = ldcyc, fl = ldfl;
            if(dp[mask][cyc][fl] != cost) continue;
            if(fl == 0) {
                for(auto& bit : bits) {
                    if(bit & mask) continue;
                    ld ncost = getCost(time, bit) * mul[cyc];
                    ll ncyc = (ll)(ncost + cyc) % sz(mul);
                    push(mask | bit, ncyc, !fl, cost + ncost);
                }
            }
            if(fl == 1) {
                for(auto& bit : bits) {
        ...
        ld res = INF;
        rep(i,0,M) res = min(res, dp[done-1][i][1]);
