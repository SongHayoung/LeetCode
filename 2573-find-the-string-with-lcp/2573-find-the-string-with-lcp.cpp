...
public:
    string findTheString(vector<vector<int>>& lcp) {
        ...
        rep(i,0,n) {
            rep(j,0,n) {
                if(i and j) {
                    ...
                    } else if(lcp[i-1][j-1] != 0 and lcp[i][j] != lcp[i-1][j-1] - 1) {
                        return "";
                    }
                }
                if(lcp[i][j] == 0) continue;
                if(i == 0 or j == 0) {
                    rep(k,0,lcp[i][j]) uni(i+k,j+k);
                } else if(i and j and lcp[i-1][j-1] == 0) {
                    rep(k,0,lcp[i][j]) uni(i+k,j+k);
                }
        ...
        rep(i,0,n) {
            ll cnt = 0;
            for(ll pi = i, pj = 0; pi < n and pj < n; pi++, pj++) {
                if(res[pi] != res[pj]) break;
                cnt += 1;
            }
            if(lcp[i][0] != cnt) return "";
        }
        rep(j,0,n) {
            ll cnt = 0;
            for(ll pi = 0, pj = j; pi < n and pj < n; pi++, pj++) {
                if(res[pi] != res[pj]) break;
                cnt += 1;
            }
            if(lcp[0][j] != cnt) return "";
        }
