...
ll dy[8]{-1,-1,-1,0,1,1,1,0}, dx[8]{-1,0,1,1,1,0,-1,-1};
umll work(vvi& A, ll Y, ll X) {
    umll res;
    ll n = sz(A), m = sz(A[0]);
    rep(i,0,8) {
        ll y = Y, x = X, now = 0;
        while(0 <= y and y < n and 0 <= x and x < m) {
            now = now * 10 + A[y][x];
            res[now] += 1;
            y += dy[i], x += dx[i];
        }
...
void init() {
    for(ll i = 2; i * i < MAX_N; i++) {
        if(sieve[i]) continue;
        for(ll j = i * i; j < MAX_N; j += i) sieve[j] = true;
    }
}
class Solution {
public:
    int mostFrequentPrime(vector<vector<int>>& A) {
        ...
        ll n = sz(A), m = sz(A[0]);
        rep(i,0,n) rep(j,0,m) {
            umll vals = work(A,i,j);
            for(auto& [k,v] : vals) freq[k] += v;
        }
        ll res = -1;
        umll primes;
        for(auto& [k,v] : freq) {
            ...
            if(sieve[k]) continue;
            if(res == -1) res = k;
            else if(freq[res] == v) res = max(res, k);
