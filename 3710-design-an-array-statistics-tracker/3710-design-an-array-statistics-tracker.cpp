...
struct Med {
    ...
    long long med() const {
        if (L.empty()) return 0;
        if((sz(L) + sz(R)) & 1) return *std::prev(L.end());
        return *begin(R);
    }

    long long countLEMed() const {
        return (long long)L.size();
    }
    long long sumLEMed() const {
        return sumL;
    }
    long long countGMed() const {
        return (long long)R.size();
    }
    long long sumGMed() const {
        return sumR;
    }
};
...
public:
    ...
    void addNumber(int number) {
        ...
        med->insert(number);
        if(freq[number]) {
            ord[freq[number]].erase(number);
        }
        ++freq[number];
        best = max(best, freq[number]);
        ord[freq[number]].insert(number);
    ...
    void removeFirstAddedNumber() {
        ...
        cnt--;
        med->pop(dq[0]);
        ord[freq[dq[0]]].erase(dq[0]);
        if(best == freq[dq[0]] and ord[freq[dq[0]]].size() == 0) best--;
        --freq[dq[0]];

        if(freq[dq[0]]) {
            ord[freq[dq[0]]].insert(dq[0]);
        }
