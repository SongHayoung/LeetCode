...
class Solution {
    ll helper(ll ma, ll limit) {
        if(ma < 10) {
            return min(ma + 1, limit + 1);
        }
        ll res = 1;
        ...
        if(s[0] - '0' > limit) return res;
        return res + helper(stoll(s.substr(1)), limit);
    }
    ll helper(ll x, ll limit, string s) {
        {
            ll val = stoll(s);
            if(x < val) return 0;
        }
        ...
        string tail = "";
        rep(i,0,sz(s)) {
            tail.push_back(X.back()); X.pop_back();
        }
        reverse(all(tail));
        if(sz(X) == 0) X = "0";
        ll res = stoll(X) + 1;
        ...
        ll tt = stoll(tail);
        if(tt < ss) res -= 1;
        return helper(res - 1, limit);
    }
public:
    long long numberOfPowerfulInt(long long start, long long finish, int limit, string s) {
        rep(i,0,sz(s)) {
            if(s[i] - '0' > limit) return 0;
        }
