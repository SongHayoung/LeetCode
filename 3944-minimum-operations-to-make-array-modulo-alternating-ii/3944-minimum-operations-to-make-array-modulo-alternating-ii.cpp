...
struct Matrix {
    ...
    Matrix operator*(const Matrix& B) const {
        ...
        rep(i, 0, r) rep(k, 0, c) {
            ...
            rep(j, 0, B.c) res[i][j] = (res[i][j] + matrix[i][k] * B.matrix[k][j]) % MOD;
        }
...
class Solution {
    vector<pair<long long, long long>> helper(vector<int>& A, int k) {
        deque<int> freq(k);
        for(auto& n : A) freq[n%k]++;
        vector<pair<long long, long long>> cands;
        long long target = k / 2;
        long long left = 0, leftSum = 0, right = 0, rightSum = 0;
        for(int i = 0; i < target; i++) {
            left += freq[i];
            leftSum += (target - i) * freq[i];
        }
        for(int i = target + 1; i < k; i++) {
            right += freq[i];
            rightSum += (i - target) * freq[i];
        }
        for(int loop = 0; loop < k; loop++) {
            long long val = (target + loop) % k;
            long long cost = leftSum + rightSum;
            cands.push_back({cost,val});
            sort(begin(cands), end(cands));
            if(cands.size() > 2) cands.pop_back();

            long long nleft = left - freq[loop] + freq[(target + loop) % k];
            long long nleftSum = leftSum - target * freq[loop] + nleft;

            long long nright = right - freq[(loop + target + 1) % k] + freq[loop];
            long long nrightSum = rightSum - right + (k - target - 1) * freq[loop];

            left = nleft, leftSum = nleftSum;
            right = nright, rightSum = nrightSum;
        }
        return cands;
    }
public:
    long long minOperations(vector<int>& nums, int k) {
        ...
        vector<pair<long long,long long>> odd = helper(o,k), even = helper(e,k);
        long long res = LLONG_MAX;
        for(auto& [cost1, val1] : odd) {
            for(auto& [cost2,val2] : even) {
                if(val1 != val2) {
