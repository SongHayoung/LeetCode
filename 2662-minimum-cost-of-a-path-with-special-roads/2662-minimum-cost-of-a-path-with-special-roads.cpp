...
public:
    int minimumCost(vector<int>& start, vector<int>& target, vector<vector<int>>& E) {
        ...
        unordered_map<pll, ll, PairHash> id;
        ll idd = 0;
        auto assign = [&](ll y, ll x) {
            if(!id.count({y,x})) {
                id[{y,x}] = idd++;
            }
        };

        auto len = [&](ll y1, ll x1, ll y2, ll x2) {
            return abs(y1-y2) + abs(x1-x2);
        };
        auto connect = [&](ll y1, ll x1, ll y2, ll x2, ll w) {
            ll u = id[{y1,x1}], v = id[{y2,x2}];
            adj[u].push_back({v,w});
            adj[v].push_back({u,len(y1,x1,y2,x2)});
        };
        assign(start[0], start[1]);
        assign(target[0], target[1]);
        for(int i = 0; i < E.size(); i++) {
            ll y1 = E[i][0], x1 = E[i][1], y2 = E[i][2], x2 = E[i][3], w = E[i][4];
            assign(y1,x1);
            assign(y2,x2);
            connect(y1,x1,y2,x2,min(len(y1,x1,y2,x2),w));
            connect(y1,x1,start[0],start[1],len(y1,x1,start[0],start[1]));
            connect(y1,x1,target[0],target[1],len(y1,x1,target[0],target[1]));
            
            connect(y2,x2,start[0],start[1],len(y2,x2,start[0],start[1]));
            connect(y2,x2,target[0],target[1],len(y2,x2,target[0],target[1]));
        }
        connect(start[0],start[1],target[0],target[1],len(start[0],start[1],target[0],target[1]));
        for(int i = 0; i < E.size(); i++) {
            ll y1 = E[i][0], x1 = E[i][1], y2 = E[i][2], x2 = E[i][3], w = E[i][4];
            for(int j = i + 1; j < E.size(); j++) {
                ll y11 = E[j][0], x11 = E[j][1], y22 = E[j][2], x22 = E[j][3], w2 = E[j][4];
                connect(y1,x1,y11,x11,len(y1,x1,y11,x11));
                connect(y1,x1,y22,x22,len(y1,x1,y22,x22));
                connect(y2,x2,y11,x11,len(y2,x2,y11,x11));
                connect(y2,x2,y22,x22,len(y2,x2,y22,x22));
            }
        }
        unordered_map<ll,ll> vis;
        priority_queue<pll,vpll,greater<pll>> q;
        auto push = [&](ll id, ll w) {
            if(!vis.count(id)) vis[id] = INF;
            if(vis[id] > w) {
                vis[id] = w;
                q.push({w,id});
            }
        };
        push(id[{start[0],start[1]}], 0);
        while(sz(q)) {
        ...
        return vis[id[{target[0],target[1]}]];
    }
};
