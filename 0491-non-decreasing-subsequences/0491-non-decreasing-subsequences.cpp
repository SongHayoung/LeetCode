...
class Solution {
    void helper(vector<vector<int>>& res, vector<pair<int,int>>& st, int pos, vector<int>& now) {
        if(pos == st.size()) {
            if(now.size() >= 2) res.push_back(now);
            return;
        }
        helper(res,st,pos+1,now);
        if(now.size() == 0 or now.back() <= st[pos].first) {
            for(int i = 1; i <= st[pos].second; i++) {
                now.push_back(st[pos].first);
                helper(res,st,pos+1,now);
            }
            for(int i = 0; i < st[pos].second; i++) {
                now.pop_back();
            }
        }
    }
public:
    vector<vector<int>> findSubsequences(vector<int>& nums) {
        ...
        for(auto& n : nums) {
            if(st.size() == 0 or st.back().first != n) st.push_back({n,1});
            else st.back().second++;
        }
        vector<int> now;
