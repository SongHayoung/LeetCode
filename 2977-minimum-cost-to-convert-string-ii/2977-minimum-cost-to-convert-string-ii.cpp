...
#define uniq(a) sort(all(a)), S.erase(unique(all(a)),end(a))
...
struct Trie {
    ...
    void insert(string& s, ll who, ll p = 0) {
        if(sz(s) == p) id = who;
        else {
            if(!next[s[p]-'a']) next[s[p]-'a'] = new Trie();
            next[s[p]-'a']->insert(s,who,p+1);
        }
    }
};
class Solution {
public:
    long long minimumCost(string& s, string& t, vector<string>& o, vector<string>& c, vector<int>& cost) {
        ...
        vvll adj;
        {
            unordered_map<string, ll> smp;
            ll id = 0;
            rep(i,0,sz(o)) {
                if(smp.count(o[i])) continue;
                smp[o[i]] = id;
                trie->insert(o[i],id);
                id += 1;
            }
            rep(i,0,sz(c)) {
                if(smp.count(c[i])) continue;
                smp[c[i]] = id;
                trie->insert(c[i],id);
                id += 1;
            }
            adj = vvll(id,vll(id,1e15));
            rep(i,0,sz(o)) {
                auto u = smp[o[i]], v = smp[c[i]];
                adj[u][v] = min(adj[u][v],cost[i] * 1ll);
            }
            rep(i,0,id) adj[i][i] = 0;
            rep(k,0,id) rep(i,0,id) {
                if(adj[i][k] == 1e15) continue;
                rep(j,0,id) {
        ...
        rep(i,0,sz(s)) {
            if(!ok[i]) continue;
            Trie* src = trie, *dst = trie;
            rep(j,i,sz(s)) {
                if(!src->next[s[j]-'a']) break;
                if(!dst->next[t[j]-'a']) break;
                src = src->next[s[j]-'a'];
                dst = dst->next[t[j]-'a'];
                if(src->id != -1 and dst->id != -1 and adj[src->id][dst->id] != 1e15) {
                    dp[j+1] = min(dp[j+1], dp[i] + adj[src->id][dst->id]);
                    ok[j+1] = true;
                }
            }
            if(s[i] == t[i]) {
