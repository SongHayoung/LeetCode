...
public:
    int maxSelectedElements(vector<int>& A) {
        sort(all(A));
        vector<pair<ll,vll>> st;
        st.push_back({false, {A[0]}});
        rep(i,1,sz(A)) {
            if(A[i] == st.back().second.back()) st.back().first = true;
            else if(A[i] == st.back().second.back() + 1) st.back().second.push_back(A[i]);
            else st.push_back({false, {A[i]}});
        }
        ll res = 0, l = 0, r = 0, n = sz(st);
        while(r < n) {
            while(r + 1 < n and st[r].second.back() + 2 == st[r+1].second.front() and st[r].first) {
                res = max(res, st[r].second.back() - st[r].second.front() + 1 + st[r].first);
                r++;
            }
            if(l != r) res = max(res, (st[r].second.back() - st[l].second.front() + st[r].first + st[l].first));
            res = max(res, st[r].second.back() - st[r].second.front() + 1 + st[r].first);
            res = max(res, st[l].second.back() - st[l].second.front() + 1 + st[l].first);
            if(r + 1 == n) break;
            if(st[r].second.back() + 2 == st[r+1].second.front()) {
                l = r;
                r = r + 1;
            } else {
                l = r = r + 1;
