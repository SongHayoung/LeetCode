...
struct Seg{
    ll mi,ma,cnt;
    Seg *le, *ri;
    Seg(vi& A, ll l, ll r) : mi(A[l]), ma(A[r]), cnt(-INF), le(nullptr), ri(nullptr) {
        if(l ^ r) {
            ll m = l + (r - l) / 2;
            le = new Seg(A,l,m);
            ri = new Seg(A,m+1,r);
    ...
    ll query(ll n) {
        if(ma < n) return cnt;
        if(mi >= n) return -INF;
        return max(le->query(n), ri->query(n));
    }
...
public:
    int maxProfit(vector<int>& prices, vector<int>& profits) {
        ...
        Seg* dp1 = new Seg(S,0,sz(S) - 1);
        Seg* dp2 = new Seg(S,0,sz(S) - 1);
        ll res = -INF;
        rep(i,0,sz(prices)) {
            res = max(res, dp2->query(prices[i]) + profits[i]);
            dp2->update(prices[i], dp1->query(prices[i]) + profits[i]);
            dp1->update(prices[i], profits[i]);
        }
        return max(res, -1ll);
    }
};