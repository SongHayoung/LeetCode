...
public:
    int largestPathValue(string colors, vector<vector<int>>& edges) {
        ...
        ll res = 0;
        queue<ll> q;
        rep(i,0,sz(colors)) if(!ind[i]) {
            q.push(i);
        }
        while(sz(q)) {
            auto u = q.front(); q.pop();
            counts[u][colors[u]-'a']++;
            res = max(res, *max_element(all(counts[u])));
            for(auto& v : adj[u]) {
                if(--ind[v] == 0) q.push(v);
                rep(i,0,26) counts[v][i] = max(counts[v][i], counts[u][i]);
            }
        }
        rep(i,0,sz(colors)) if(ind[i]) return -1;
        return res;
    }
};
