...
public:
    long long maximumProfit(vector<int>& prices, int k) {
        vvll dp(3,vll(k + 1,-INF));
        dp[0][0] = 0;
        rep(i,0,sz(prices)) {
            vll norm = dp[1], shor = dp[2];
            rep(trans,0,k) {
                norm[trans] = max(norm[trans], dp[0][trans] - prices[i]);
                shor[trans] = max(shor[trans], dp[0][trans] + prices[i]);
            }
            rep(trans,0,k) {
                dp[0][trans+1] = max({dp[0][trans+1], dp[1][trans] + prices[i], dp[2][trans] - prices[i]});
            }
            dp[1] = norm, dp[2] = shor;
        }
        ll res = 0;
        rrep(i,0,k) res = max(res, dp[0][i]);
        return res;
    }
};
