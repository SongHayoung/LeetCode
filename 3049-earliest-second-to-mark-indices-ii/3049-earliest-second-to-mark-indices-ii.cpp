...
class Solution {
    bool helper(vi& A, vi& B, ll m) {
        if(m == sz(B) + 1) return true;
        vb fl(sz(A));
        vpll ps;
        rep(i,0,m) {
            if(!fl[B[i] - 1]) {
                fl[B[i] - 1] = true;
                ps.push_back({i, B[i] - 1});
            }
        }
        ll k = sz(ps) - 1, rem = 0;
        priority_queue<pll> q;
        rrep(i,0,m-1) {
            if(k >= 0 and ps[k].first == i) {
                if(A[ps[k].second] > 0) {
                    q.push({-A[ps[k].second], ps[k].second});
                } else rem++;
                k--;
            } else rem++;
            while(rem < sz(q)) {
