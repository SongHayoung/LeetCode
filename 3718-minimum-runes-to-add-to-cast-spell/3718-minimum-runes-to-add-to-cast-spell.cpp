...
ll vis[MAX_N], rep[MAX_N];
vll st;
void dfs1(vvll& adj, ll u) {
    vis[u] = true;
    for(auto& v : adj[u]) {
        if(!vis[v]) dfs1(adj,v);
    }
...
void dfs2(vvll& radj, ll u, ll root) {
    vis[u] = true;
    rep[u] = root;
    for(auto& v : radj[u]) {
        if(!vis[v]) dfs2(radj,v,root);
    }
}
class Solution {
public:
    int minRunesToAdd(int n, vector<int>& crystals, vector<int>& flowFrom, vector<int>& flowTo) {
        ...
        usll roots;
        unordered_map<ll,vll> gr;
        rep(i,0,n) gr[rep[i]].push_back(i);
        for(auto& [k,nodes] : gr) {
            bool ok = true;
            for(auto& u : nodes) {
                for(auto& v : radj[u]) {
            ...
            if(ok) roots.insert(k);
        }
        rep(i,0,sz(crystals)) {
            roots.erase(rep[crystals[i]]);
        }
        return sz(roots);
    }
};
