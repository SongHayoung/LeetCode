...
struct Seg {
    ll mi, ma, fl;
    Seg *left, *right;
    Seg(vb& A, ll l, ll r) : mi(l), ma(r), fl(0), left(nullptr), right(nullptr) {
        if(l ^ r) {
            ll m = l + (r - l) / 2;
            left = new Seg(A,l,m);
    ...
    bool query(ll l, ll r) {
        if(l > r) return true;
        if(l <= mi and ma <= r) return fl;
        if(mi > r or ma < l) return true;
...
class Solution {
    bool helper(Seg* seg, vvll& freq1, vvll& freq2, vi& Q, ll n) {
        ll a = Q[0], b = Q[1], d = n - Q[2] - 1, c = n - Q[3] - 1;
        if((a <= c and c <= b) or (a <= d and d <= b) or (c <= b and b <= d) or (c <= a and a <= d)){
            ll mi = min(a,c), ma = max(b,d);
            if(!seg->query(0,mi-1)) return false;
            if(!seg->query(ma+1,sz(freq1))) return false;
            if((a <= c and d <= b) or (c <= a and b <= d)) {
                rep(i,0,26) {
                    if(freq1[ma+1][i] - freq1[mi][i] != freq2[ma+1][i] - freq2[mi][i]) return false;
                }
                return true;
            }
            ll mi2 = max(a,c), ma2 = min(b,d);
            ll cnt = ma2 - mi2 + 1;
            vll ra(26), rb(26);
            rep(i,0,26) {
                ra[i] = freq1[b+1][i] - freq1[a][i] - freq2[b+1][i] + freq2[a][i];
                rb[i] = freq2[d+1][i] - freq2[c][i] - freq1[d+1][i] + freq1[c][i];
                ra[i] += (freq2[ma2+1][i] - freq2[mi2][i]);
                rb[i] += (freq1[ma2+1][i] - freq1[mi2][i]);
                if(ra[i] < 0 or rb[i] < 0) return false;
            }

            return ra == rb;
        }
        rep(i,0,26) {
            if(freq1[b+1][i] - freq1[a][i] != freq2[b+1][i] - freq2[a][i]) return false;
            if(freq1[d+1][i] - freq1[c][i] != freq2[d+1][i] - freq2[c][i]) return false;
        }
        ll mi = min(a,c), ma = max(b,d);
        if(!seg->query(0,mi-1)) return false;
        if(!seg->query(ma+1,sz(freq1))) return false;
        mi = min(b,d), ma = max(a,c);
        if(seg->query(mi+1,ma-1)) return true;
        return false;
    }
public:
    vector<bool> canMakePalindromeQueries(string s, vector<vector<int>>& queries) {
        ...
        rep(i,0,x) {
            ...
            s1.push_back(s[i]);
            s2.push_back(s[j]);
            fl.push_back(s1.back() == s2.back());
        }
        Seg* seg = new Seg(fl,0,sz(fl)-1);
        vvll freq1(sz(s1)+1, vll(26));
        vvll freq2(sz(s2)+1, vll(26));
        vll now1(26), now2(26);
        rep(i,0,sz(s1)) {
            now1[s1[i]-'a'] += 1;
            now2[s2[i]-'a'] += 1;
        ...
        rep(i,0,sz(queries)) {
            res.push_back(helper(seg,freq1, freq2,queries[i],sz(s)));
        }
