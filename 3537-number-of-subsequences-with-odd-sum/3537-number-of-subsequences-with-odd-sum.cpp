...
public:
    int subsequenceCount(vector<int>& nums) {
        ...
        ll res = 0, mod = 1e9 + 7;
        auto mul = [&](pll a, ll x) {
            return pll{a.first, (a.second + a.first * x) % mod};
        };
        auto merge = [&](pll& A, pll& B) {
        ...
        for(auto& n : nums) {
            pll o,e;
            if(n & 1) {
                o = mul(even,n),e = mul(odd,n);
                o.first += 1, o.second += n;
            } else {
                o = mul(odd, n), e = mul(even, n);
                e.first += 1, e.second += n;
            }
            merge(odd,o);
            merge(even,e);
            res = (res + o.first) % mod;
