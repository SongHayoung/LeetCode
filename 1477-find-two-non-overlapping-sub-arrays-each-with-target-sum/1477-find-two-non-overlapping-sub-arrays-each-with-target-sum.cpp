...
public:
    int minSumOfLengths(vector<int>& A, int target) {
        vpll st;
        {
            ll sum = 0, l = sz(A) - 1;
            rrep(i,0,sz(A) - 1) {
                while(l >= 0 and sum < target) sum += A[l--];
                if(sum == target) {
                    ll len = i - l;
                    if(sz(st) == 0 or st.back().second > len) st.push_back({l+1,len});
                }
                sum -= A[i];
            }
        }
        ...
        ll sum = 0, r = 0;
        rep(i,0,sz(A) - 1) {
            while(r < sz(A) and sum < target) sum += A[r++];
            if(sum == target) {
                while(sz(st) and st.back().first < r) st.pop_back();
                if(sz(st)) {
                    res = min(res, r - i + st.back().second);
                }
            }
            sum -= A[i];
        }
