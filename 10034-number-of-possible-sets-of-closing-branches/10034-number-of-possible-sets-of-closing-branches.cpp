...
public:
    int numberOfSets(int n, int maxDistance, vector<vector<int>>& roads) {
        vvll base(n,vll(n,INF));
        rep(i,0,n) base[i][i] = 0;
        rep(i,0,sz(roads)) {
            auto u = roads[i][0], v = roads[i][1], w = roads[i][2];
            base[u][v] = base[v][u] = min(base[u][v], 1ll * w);
        }
        int res = 0;
        rep(mask,0,1ll<<n) {
            vll open;
            rep(j,0,n) if(BIT(mask,j)) open.push_back(j);
            vvll dis(n,vll(n,INF));
            rep(j,0,sz(open)) rep(k,0,sz(open)) {
                dis[open[j]][open[k]] = base[open[j]][open[k]];
            }
            for(auto& k : open) for(auto& u : open) for(auto& v : open) {
                dis[u][v] = min(dis[u][v], dis[u][k] + dis[k][v]);
            }
            bool ok = true;
            rep(j,0,sz(open)) {
                rep(k,j+1,sz(open)) {
                    if(dis[open[j]][open[k]] > maxDistance) ok = false;
                    if(!ok) break;
                }
                if(!ok) break;
            }
