...
public:
    vector<vector<int>> modifiedGraphEdges(int n, vector<vector<int>>& edges, int source, int destination, int target) {
        ...
        auto push = [&](ll u, ll cost, bool force = false) {
            if(vis[u] > cost or force) {
                vis[u] = cost;
                q.push({cost,u});
            }
        };
        ...
        dijkstra();
        if(vis[destination] < target) return {};
        while(sz(bad) and vis[destination] > target) {
            auto idx = bad.back(); bad.pop_back();
            ll u = edges[idx][0], v = edges[idx][1], w = edges[idx][2] = 1;
            adj[u].push_back({v,w});
            adj[v].push_back({u,w});
            push(u,vis[u],true);
            push(v,vis[v],true);
            ...
            if(vis[destination] <= target) {
                edges[idx][2] = target - vis[destination] + 1;
            }
        }
        if(vis[destination] <= target) return edges;
        return {};
    }
};
