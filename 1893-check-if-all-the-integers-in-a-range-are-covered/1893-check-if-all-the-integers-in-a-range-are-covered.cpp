...
class Solution {
    bool dup(vector<int> A, vector<int> B) {
        if(A[0] > B[0]) swap(A,B);
        return A[1] >= B[0];
    }
    bool check(vector<vector<int>>& ranges, int left, int right) {
        ll tot = ranges.size();
        map<int,ll> c,o;
        map<long,ll> ro;
        rep(i,0,sz(ranges)) {
            ll l = ranges[i][0], r = ranges[i][1];
            ro[-1L * l]++,c[r]++;
        }
        {
            ll sum = 0;
        ...
        ll cov = tot;
        auto oit = o.upper_bound(right);
        if(oit != end(o)) {
            cov -= oit->second;
        }
        auto cit = c.lower_bound(left);
        if(cit != begin(c)) {
        ...
        cout<<cov<<endl;
        for(int i = 0; i < tot; i++)
            if(dup(ranges[i], {left, right})) cov--;
        return cov == 0;
    }
public:
    bool isCovered(vector<vector<int>>& ranges, int left, int right) {
        bool t = true, f = false;
        if(!check(ranges,left,right)) swap(t,f);
        int line[52] = {};
        for (auto &r : ranges) {
            line[r[0]] += 1;
            line[r[1] + 1] -= 1;
