...
struct Trie {
    ...
    void del(string& s, ll p = 0) {
        sub--;
        if(p == sz(s)) {
            match -= 1;
            return;
        }
        next[s[p]-'a']->del(s,p+1);
    }
    ll query(string& s,dqll& at,ll p = 0) {
        if(!sub) return 0;
        if(sz(at) == 0) return 0;
        ll res = 0;
        if(at.front() == p) {
        ...
        if(p == sz(s)) return res;
        if(!next[s[p]-'a']) return res;
        return res + next[s[p]-'a']->query(s,at,p + 1);
    }
};
class Solution {
public:
    ll countPrefixSuffixPairs(vector<string>& A) {
        Trie* t = new Trie();
        rep(i,0,sz(A)) t->insert(A[i]);
        ll res = 0;
        rrep(i,0,sz(A)-1) {
            vll z = zfunction(A[i]);
            t->del(A[i]);
