...
class Solution {
    vvll adj;
    array<dqll,3> dfs(ll u, ll par, vll& res, vi& cost) {
        dqll neg, pos, zero;
        
        if(cost[u] == 0) zero.push_back(0);
        else if(cost[u] > 0) pos.push_back(cost[u]);
        else if(cost[u] < 0) neg.push_back(cost[u]);
        for(auto& v : adj[u]) {
            if(v == par) continue;
            ...
            while(sz(zero) > 3) zero.pop_back();
        }
        if(sz(neg) + sz(pos) + sz(zero) < 3) res[u] = 1;
        else {
            if(sz(neg) >= 2 and sz(pos)) {
                res[u] = neg[0] * neg[1] * pos.back();
            }
            if(sz(pos) >= 3) {
                res[u] = max(res[u], pos[0] * pos[1] * pos[2]);
            }
        }
        return {neg,pos,zero};
    }
