...
public:
    long long subArrayRanges(vector<int>& A) {
        ...
        vll mi,ma;
        rep(i,0,sz(A)) {
            while(sz(mi) and A[mi.back()] >= A[i]) {
                ll pos = mi.back(); mi.pop_back();
                ll le = sz(mi) ? mi.back() : -1;
                ll ri = i;
                res -= (pos - le) * (ri - pos) * A[pos];
            }
            while(sz(ma) and A[ma.back()] <= A[i]) {
                ll pos = ma.back(); ma.pop_back();
                ll le = sz(ma) ? ma.back() : -1;
                ll ri = i;
                res += (pos - le) * (ri - pos) * A[pos];
            }
            mi.push_back(i);
