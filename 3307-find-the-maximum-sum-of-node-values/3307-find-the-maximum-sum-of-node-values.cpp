...
class Solution {
    vvll adj;
    pll dfs(ll u, ll par, ll k, vi& A) {
        vall3 ord;
        for(auto& v : adj[u]) {
            if(v == par) continue;
            auto [n,y] = dfs(v,u,k,A);
            ord.push_back({n,y,v});
        }
        if(sz(ord) == 0) return {A[u], -INF};
        vall3 best;
        for(auto& [n,y,v] : ord) {
            ll conn = max(n - A[v] + (A[v] ^ k), y - (A[v] ^ k) + A[v]);
            ll nconn = max(n,y);
            best.push_back({conn, nconn, v});
        }
        ll no = 0, yes = 0;
        sort(all(best), [](auto a, auto b) {
            return a[0] - a[1] > b[0] - b[1];
        });
        rep(i,0,sz(best)) no += best[i][1];
        yes = no - best[0][1] + best[0][0];
        ll sum = no;
        for(ll i = 0; i + 1 < sz(best); i += 2) {
            sum = sum + best[i][0] + best[i+1][0] - best[i][1] - best[i+1][1];
            no = max(no, sum);
        }
        sum = yes;
        for(ll i = 1; i + 1 < sz(best); i += 2) {
            sum = sum + best[i][0] + best[i+1][0] - best[i][1] - best[i+1][1];
            yes = max(yes, sum);
        }
...
public:
    long long maximumValueSum(vector<int>& nums, int k, vector<vector<int>>& E) {
        ...
        adj = vvll(n);
        rep(i,0,sz(E)) {
            auto u = E[i][0], v = E[i][1];
            adj[u].push_back(v);
            adj[v].push_back(u);
        }
        auto [a,b] = dfs(0,-1,k,nums);
        return max(a,b);
    }
};
