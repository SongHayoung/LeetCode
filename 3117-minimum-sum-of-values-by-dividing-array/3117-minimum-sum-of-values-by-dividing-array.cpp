        ...
        rep(i,0,n) {
            ...
            rep(j,0,m) {
                for(auto& [k,v] : dp[j]) {
                    ll nbit = k & A[i];
                    if(nbit == Q[j]) {
                        if(!dpp[j+1].count(INT_MAX)) dpp[j+1][INT_MAX] = INF;
                        dpp[j+1][INT_MAX] = min(dpp[j+1][INT_MAX], v + A[i]);
                    }
                    if(!dpp[j].count(nbit)) dpp[j][nbit] = INF;
                    dpp[j][nbit] = min(dpp[j][nbit], v);
                }
            }
            swap(dp,dpp);
        }
        if(dp[m].count(INT_MAX)) return dp[m][INT_MAX];
        return -1;
    }
};