...
ll comb(ll n, ll r) {
    ...
    ll now = comb(n-1,r-1) + comb(n-1,r);
    if(now > lim) now = lim;
    return binom[n][r] = now;
}
class Solution {
    ll helper(ll bit, ll x, ll base) {
        ull res = (bit + 1) % x == 0;
        ll pre = 0, on = 0;
        rep(i,1,bit+1) {
            if(i % x == 0) on += 1;
        ...
        ull sum = 1ll<<pre;
        rep(i,1,on+1) {
            if(res >= 1e15) return res = INF;
            ll now = i * sum, c = comb(on,i);
            if(c >= INF or now >= INF or c < 0 or c >= INF / now) return res = INF;
            if(now >= 1e15) return res = INF;
            res += now * c;
        }
        return min(res,1ull * INF);
    }
public:
    long long findMaximumNumber(long long k, int x) {
        long long res = 0, ma = 50;
        MINUS(binom);
        lim = k;
        vll pre(ma);
        rep(i,0,ma) {
