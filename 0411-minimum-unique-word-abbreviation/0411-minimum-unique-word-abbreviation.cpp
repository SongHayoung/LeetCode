...
class Solution {
    void build(vs& F, string& s, string now, ll pos) {
        if(pos == sz(s)) F.push_back(now);
        else {
            build(F,s,now + s[pos], pos + 1);
            if(pos == 0 or !isdigit(now.back())) {
                rep(j,pos+1,sz(s) + (pos != 0)) {
                    build(F,s,now + to_string(j - pos), j);
                }
            }
        }
    }
    bool match(string& p, string& s) {
        for(ll i = 0, j = 0; i < sz(p) and j < sz(s);) {
            if(isdigit(p[i])) {
                ll jump = 0;
                while(i < sz(p) and isdigit(p[i])) {
                    jump = jump * 10 + p[i]-'0';
                    i++;
                }
                j += jump;
...
public:
    string minAbbreviation(string target, vector<string>& dictionary) {
        vs D;
        rep(i,0,sz(dictionary)) if(sz(dictionary[i]) == sz(target)) D.push_back(dictionary[i]);
        if(sz(D) == 0) return to_string(sz(target));
        vs F;
        build(F,target, "", 0);
        for(auto& d : D) {
            rep(i,0,sz(F)) {
