...
struct Seg {
    ll mi, ma, best;
    Seg* left, *right;
    Seg(vll& A, ll l, ll r) : mi(A[l]), ma(A[r]), best(r ? 0 : INF), left(nullptr), right(nullptr) {
        if(l^r) {
            ll m = l + (r - l) / 2;
            left = new Seg(A,l,m);
...
public:
    vector<bool> getResults(vector<vector<int>>& Q) {
        ...
        rep(i,0,sz(Q)) {
            if(Q[i][0] == 1) {
                ll p = Q[i][1];
                auto ub = mp.upper_bound(p)->first;
                mp[p] = ub - p;
                seg->update(p,ub-p);
                ...
                ll p = Q[i][1], len = Q[i][2];
                auto it = prev(mp.upper_bound(p));
                ll best = seg->query(it->first);
                if(best >= len) {
                    res.push_back(true);
                } else {
                    if(it->first == p) it--;
