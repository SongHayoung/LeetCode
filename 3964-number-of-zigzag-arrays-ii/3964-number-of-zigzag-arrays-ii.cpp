...
public:
    int zigZagArrays(int n, int l, int r) {
        ...
        Matrix mat(d,d);
        rep(i,0,m) {
            rep(j,0,i) mat[i][m + j] = 1;
            rep(j,i+1,m) mat[m + i][j] = 1;
        }
        vll f(d);
        rep(i,0,m) f[i] = i % mod, f[i + m] = (m - 1 - i) % mod;

        Matrix po = mat.pow(n - 2);

        ll res = 0;
        rep(i,0,d) rep(j,0,d) if(po[i][j]) res = (res + po[i][j] * f[j] % mod) % mod;
        return res;
    }
};
