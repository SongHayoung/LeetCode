...
public:
    int ladderLength(string beginWord, string endWord, vector<string>& wordList) {
        ...
        rep(i,0,n) {
            if(wordList[i] == endWord) goal = i;
            rep(j,i+1,n) {
                if(ok(wordList[i], wordList[j])) adj[i].push_back(j), adj[j].push_back(i);
            }
            if(ok(beginWord, wordList[i])) push(i);
        }
        if(goal == -1) return 0;
        ll res = 2;
        while(sz(q) and !vis[goal]) {
            ll qsz = sz(q);
            while(qsz--) {
                int u = q.front(); q.pop();
