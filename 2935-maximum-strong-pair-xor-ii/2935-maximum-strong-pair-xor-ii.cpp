...
struct Trie {
    ...
    void insert(ll x, ll bit = 32) {
        cnt += 1;
        if(bit < 0) return;
        bool fl = BIT(x,bit);
        if(!next[fl]) next[fl] = new Trie();
        next[fl]->insert(x,bit-1);
    }
    void cut(ll x, ll bit = 32) {
        cnt -= 1;
        if(bit < 0) return;
        bool fl = BIT(x,bit);
        next[fl]->cut(x,bit-1);
    }
    ll query(ll x, ll bit = 32) {
        if(bit < 0) return 0;
        bool fl = BIT(x,bit);
        if(next[!fl] and next[!fl]->cnt) return (1ll<<bit) + next[!fl]->query(x,bit-1);
        return next[fl]->query(x,bit-1);
    }
};
class Solution {
public:
    int maximumStrongPairXor(vector<int>& A) {
        ...
        while(l < n) {
            while(r < n and A[l] * 2 >= A[r]) {
                trie->insert(A[r]); r += 1;
            }
            res = max(res, trie->query(A[l]));
            trie->cut(A[l]);
            l += 1l;
        }
        return res;
