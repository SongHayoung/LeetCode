...
public:
    long long minIncrementOperations(vector<int>& A, int k) {
        ...
        rep(i,0,8) {
            ...
            rep(j,0,3) {
                if(BIT(i,j)) {
                    if(A[2-j] < k) cost += k - A[2-j];
                    ok = true;
                } else if(A[2-j] >= k) ok = true;
            }
            if(ok) dp[i] = cost;
        }
        rep(i,3,sz(A)) {
            vll dpp(8,1e15);
            rep(mask,0,8) {
                ll pc = __builtin_popcountll(mask & 3);
                rep(fl,0,2) {
                    ll nmask = ((mask<<1) & 7) | fl;
                    if(fl) {
                        dpp[nmask] = min(dpp[nmask], dp[mask] + max(0,k-A[i]));
                    } else {
                        if(pc) {
                            dpp[nmask] = min(dpp[nmask], dp[mask]);
                        } else {
                            ll ma = -1;
                            rep(x,i-2,i+1) ma = max(A[x] * 1ll,ma);
                            if(ma >= k) {
                                dpp[nmask] = min(dpp[nmask], dp[mask]);
                            }
                        }
