...
class Solution {
    ...
    long long helper(unordered_map<long long, long long>& freq, vector<long long>& f, int p, long long us, long long pick) {
        if(p == f.size()) {
            if(pick == 0) return 0;
            long long res = 1;
            for(long long i = 0; i <= 30; i++) {
                if(pick & (1<<i)) res = res * freq[f[i]] % mod;
            }
            return res;
        }
        long long res = helper(freq,f,p+1,us, pick);
        long long me = factor(f[p]);
        bool ok = (me & us) == 0;
        if(ok) {
            res = (res + helper(freq,f,p+1,us | me,pick | (1ll<<p))) % mod;
        }
        return res;
    }
public:
    int squareFreeSubsets(vector<int>& nums) {
        ...
        vector<long long> f;
        for(auto [k,v] : freq) if(k != 1) f.push_back(k);
        long long res = helper(freq,f,0,0,0);
        if(freq.count(1)) {
            res = ((res + 1) * modpow(2,freq[1]) % mod - 1 + mod) % mod;
        }
        return res;
    }
};
