...
public:
    ...
    void push(int x) {
        st.push_back(x);
        ord[x].push_back(sz(st)-1);
    }
    void clean() {
        while(sz(us) and us.count(sz(st) - 1)) {
            st.pop_back();
            us.erase(sz(st));
        }
    }
    ...
    int pop() {
        ll res = top(); st.pop_back();
        ord[res].pop_back();
        if(sz(ord[res]) == 0) ord.erase(res);
        return res;
    }

    int peekMax() {
        clean();
        return prev(end(ord))->first;
    }

    int popMax() {
        ll res = peekMax();
        us.insert(ord[res].back());
        ord[res].pop_back();
        if(sz(ord[res]) == 0) ord.erase(res);
        clean();
        return res;
