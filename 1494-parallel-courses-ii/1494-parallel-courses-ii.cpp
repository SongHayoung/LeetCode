...
class Solution {
    ll dp[1ll<<16], near[16], tot;
    vll extract(ll mask) {
        vll res;
        rep(i,0,tot) {
            if(BIT(mask,i)) continue;
            if((near[i] & mask) == near[i]) res.push_back(i);
        }
        return res;
    }
    void backTracking(vll& A, ll k, ll pos, ll mask, vll& res) {
        if(pos == sz(A) or !k) res.push_back(mask);
        else {
            if(sz(A) - pos > k) backTracking(A,k,pos+1,mask,res);
            backTracking(A,k-1,pos+1,mask | (1ll<<A[pos]), res);
        }
    }
    ll dfs(ll mask, ll k) {
        if(dp[mask] != -1) return dp[mask];
        vll cand = extract(mask);
        vll nbits;
        backTracking(cand, k, 0, 0, nbits);
        ll& res = dp[mask] = INF;
        for(auto& bit : nbits) {
            res = min(res, 1 + dfs(mask | bit, k));
        }
        return res;
    }
public:
