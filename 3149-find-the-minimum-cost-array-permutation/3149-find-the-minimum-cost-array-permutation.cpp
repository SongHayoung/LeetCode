...
ll id;
ll dppp[1ll<<15][15][15];
ll dp[1ll<<15][15][15];
ll dpp[1ll<<15][15][15];
class Solution {
    ll helper(vvll& A, ll mask, ll x, ll st) {
        ll n = sz(A), fin = (1ll<<n) - 1;
        if(mask == fin) return A[x][st];
        if(dppp[mask][x][st] == id) return dp[mask][x][st];
        dppp[mask][x][st] = id;
        ll& res = dp[mask][x][st] = INF;
        ll& go = dpp[mask][x][st];
        rep(i,0,n) {
            if(BIT(mask,i)) continue;
            ll sub = helper(A,mask | (1ll<<i), i, st) + A[x][i];
            if(sub < res) {
                res = sub;
                go = i;
            }
...
public:
    vector<int> findPermutation(vector<int>& A) {
        ...
        rep(i,0,n) {
            rep(j,0,n) {
                if(i == j) dis[i][j] = INF;
                else dis[i][j] = abs(i - A[j]);
            }
        }
        vi res;
        ll score = INF, st = 0;
        rep(i,0,n) {
            helper(dis,1ll<<i,i,i);
            if(dp[1ll<<i][i][i] < score) {
                st = i;
                score = dp[1ll<<i][i][i];
            }
        }
        ll mask = 1ll<<st, at = st;
        rep(i,0,n) {
            res.push_back(at);
            ll v = dpp[mask][at][st];
            mask |= 1ll<<v;
