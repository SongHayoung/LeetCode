...
ll dp[1<<15][15];
class Solution {
    ll helper(vector<int>& A, ll mask, ll p) {
        if(mask == (1ll<<sz(A)) - 1) return 1;
        if(dp[mask][p] != -1) return dp[mask][p];
        ll& res = dp[mask][p] = 0;
        rep(i,0,sz(A)) {
            if(BIT(mask,i)) continue;
            if(A[p] % A[i] == 0 or A[i] % A[p] == 0) {
                res = (res + helper(A,mask | (1<<i), i)) % mod;
            }
        }
        return res;
    }
