...
class Solution {
public:
    vector<vector<int>> resultGrid(vector<vector<int>>& A, int k) {
        ll n = sz(A), m = sz(A[0]);
        vector<vpll> cnt(n,vpll(m,{0,0}));
        rep(i,0,n-2) rep(j,0,m-2) {
            ll sum = 0;
            bool ok = true;
            rep(x,0,3) rep(y,0,3) {
                if(x) {
                    if(abs(A[i+x][j+y] - A[i+x-1][j+y]) > k) ok = false;
                }
                if(y) {
                    if(abs(A[i+x][j+y] - A[i+x][j+y-1]) > k) ok = false;
                }
                sum += A[i+x][j+y];
            }
            if(ok) {
        ...
        rep(i,0,n) rep(j,0,m) {
            if(cnt[i][j].first) {
                res[i][j] = cnt[i][j].second / cnt[i][j].first;
            }  else res[i][j] = A[i][j];
        }
        return res;
    }
