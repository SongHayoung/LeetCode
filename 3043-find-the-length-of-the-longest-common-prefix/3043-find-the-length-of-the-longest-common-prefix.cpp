...
struct Trie {
    ...
    void insert(string& s, ll p = 0) {
        if(p == sz(s)) return;
        if(!next[s[p]-'0']) next[s[p]-'0'] = new Trie();
        next[s[p]-'0']->insert(s,p+1);
    }
    ll query(string& s, ll p = 0) {
        if(p == sz(s)) return p;
        if(!next[s[p]-'0']) return p;
        return next[s[p]-'0']->query(s,p+1);
    }
};
