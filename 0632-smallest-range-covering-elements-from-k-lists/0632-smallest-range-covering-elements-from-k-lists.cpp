...
public:
    vector<int> smallestRange(vector<vector<int>>& nums) {
        ...
        auto best = [&](int l, int r) {
            ...
            else if(r - l == res[1] - res[0]) res = min(res, vi{l,r});
        };
        ...
        bool ok = true;
        while(ok) {
            best(begin(mp)->first, prev(end(mp))->first);
            int val = begin(mp)->first;
            for(auto& who : begin(mp)->second) {
                while(idx[who] < nums[who].size() and nums[who][idx[who]] == val) {
                    idx[who]++;
                }
                if(idx[who] == nums[who].size()) ok = false;
                else {
                    mp[nums[who][idx[who]]].push_back(who);
                }
            }
            mp.erase(begin(mp));
        }
