...
ll bitcnt(ll x) {
    vll bits = count(x);
    return accumulate(all(bits),0ll);
}
ll search(ll x) {
    ll l = 0, r = 1e15, res = l;
    while(l <= r) {
...
class Solution {
    ll helper(ll l, ll r, ll mod) {
        l += 1, r += 1;
        ll le = search(l), ri = search(r);
        vll tot(62);
        vll rri = count(ri);
        vll lle = count(le);
        rep(i,0,62) tot[i] = rri[i] - lle[i];
        {
            ll cnt = bitcnt(le);
            le++;
            while(cnt < l) {
                rep(i,0,62) {
                    if(!BIT(le,i)) continue;
                    cnt++;
                    if(cnt == l) break;
                    tot[i]--;
                }
        ...
        rep(i,0,62) {
            if(tot[i]) {
                res = (res * modpow(po,tot[i], mod) % mod);
            }
            po *= 2;
        }
        return res;
...
public:
    vector<int> findProductsOfElements(vector<vector<long long>>& queries) {
        vi res;
        rep(i,0,sz(queries)) {
            res.push_back(helper(queries[i][0], queries[i][1], queries[i][2]));
        }
        return res;
    }
};