...
public:
    int maxValue(vector<vector<int>>& A, int k) {
        ...
        sort(rall(A));
        map<int, vll> mp;
        auto merge = [&](vll& dp, ll x) {
            if(!mp.count(x)) mp[x] = dp;
            else {
                auto it = mp.find(x);
                rep(i,0,k) {
                    it->second[i] = max(it->second[i], dp[i]);
                }
            }
        };
        while(A.size()) {
            ll x = A.back()[0];
            while(mp.size() and mp.begin()->first < x) {
                auto it = mp.begin();
                rep(i,0,k) {
                    best[i] = max(best[i], it->second[i]);
                }
                mp.erase(it);
            }
            while(A.size() and A.back()[0] == x) {
                ll r = A.back()[1], w = A.back()[2]; A.pop_back();
                vll now = best;
                rrep(i,0,k-2) {
                    now[i+1] = max(now[i+1], now[i] + w);
                }
                now[0] = max(now[0], w);
                merge(now,r);
            }
        ...
        for(auto& [_,vec] : mp) {
            res = max(res, *max_element(all(vec)));
        }
        return res;
    }
};
