...
class Solution {
    ...
    ll helper(ll pos, ll mask, ll k) {
        ...
        rep(i,0,26) {
            ...
            auto poss = *lower_bound(all(at[i]), pos);
            ord.push_back(poss);
        }
        sort(rall(ord));
...
public:
    int maxPartitionsAfterOperations(string& s, int k) {
        {
            ll bit = 0;
            rep(i,0,sz(s)) bit |= 1ll<<(s[i]-'a');
            if(__builtin_popcountll(bit) + 1 <= k) return 1;
        }
        ZERO(dp);
        at = vvll(26);
        {
            ...
            rrep(i,0,sz(s) - 1) {
                now[s[i]-'a'] = i;
                vll ord;
                rep(j,0,26) ord.push_back(now[j]);
                sort(all(ord));
                dp[i] = 1 + dp[ord[k]];
            }
        }
        rep(i,0,sz(s)) at[s[i]-'a'].push_back(i);
        ...
        rep(i,0,sz(s)) {
            ...
            rep(j,0,26) {
                ...
                res = max(res, nnow + helper(i+1,nbit,k));
            }
            bit |= 1ll<<(s[i]-'a');
